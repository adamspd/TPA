...
cp -f /home/ubuntu/tpa/config/hive/* "${HIVE_HOME}/conf"
...
if [[ ! -d mongo-hadoop ]]; then
    ...
    ./gradlew hive:jar
    cd ..
    chown -R ubuntu:ubuntu /usr/local/mongo-hadoop
...
__log_info 'Give ubuntu user ownership over Hive'
chown -R ubuntu:ubuntu /usr/local/${HIVE_VERSION}
...
su -l ubuntu -c 'hadoop fs -mkdir -p /tmp'
su -l ubuntu -c 'hadoop fs -mkdir -p /user/hive/warehouse'
su -l ubuntu -c 'hadoop fs -mkdir -p /secrets'
su -l ubuntu -c 'hadoop fs -chmod g+w /tmp'
su -l ubuntu -c 'hadoop fs -chmod g+w /user/hive/warehouse'
...
su -l ubuntu -c 'hadoop fs -rm -f /secrets/hive.jceks'
su -l ubuntu -c "
...
su -l ubuntu -c 'schematool -initSchema -dbType mysql'

__log_info 'Start Hive Metastore service'
su -l ubuntu -c 'nohup hive --service metastore > /dev/null &'

__log_info 'Start HiveServer2'
su -l ubuntu -c 'nohup hiveserver2 > /dev/null &'

__log_info 'Fix Hive warning appearing when starting pyspark'
python3 /home/ubuntu/tpa/config/bin/fix-spark-hive-warning.py
